...
# Added extraction from hidden layers to get features for SegFormer
# Also uses faster Flash Attention or Math kernel depending on hardware
# Works best with mixed precision training
# Based on: https://github.com/microsoft/Cream/tree/main/TinyViT
# Which is adapted from LeViT and Swin Transformer
...
class TinyViT(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.output_intermediate_features:
            x = self.norm_head(x)
            x = self.head(x)
        return x
...
@register_model
def tiny_vit_5m_240(pretrained=False, **kwargs):
    model_kwargs = dict(
        embed_dims=[60, 120, 240, 480],
        depths=[2, 2, 6, 2],
        num_heads=[2, 4, 6, 12],
        window_sizes=[8, 8, 16, 8],
        drop_path_rate=0.0,
    )
    model_kwargs.update(kwargs)
    return _create_tiny_vit("tiny_vit_5m_240", pretrained, **model_kwargs)


if __name__ == "__main__":
    model = tiny_vit_5m_224(pretrained=False, img_size=64, num_classes=0)
    print(model)

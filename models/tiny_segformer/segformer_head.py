...
class SimpleConvModule(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0,
        ...
        super().__init__()
        layers = []
        layers.append(nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, bias=not norm_layer))
...
class SegFormerHead(nn.Module):
    ...
    """
    def __init__(self, in_channels, embed_dim, num_classes, dropout_rate=0.1, norm_layer=nn.BatchNorm2d, act_layer=nn.ReLU):
        super().__init__()
        ...
        self.linear_fuse = SimpleConvModule(embed_dim * len(in_channels), embed_dim, 1, norm_layer=norm_layer, act_layer=act_layer)
    ...
    def forward(self, inputs):
        ...
        for i in range(len(inputs)):
            ...
            feat = feat.permute(0, 2, 1).reshape(B, -1, inputs[i].shape[2], inputs[i].shape[3])
            x.append(feat)
        
        # Upsample all to highest resolution
        x = [F.interpolate(i, size=(H, W), mode='bilinear', align_corners=False) for i in x]
...
if __name__ == "__main__":
    ...
    y = model(x)
    assert y.shape == (1, 1, 64, 64), f"Output shape should be (1, 1, 64, 64) got {y.shape}"
    print(y)
...
def load_data(partition_id: int, num_partitions: int, run_config: Dict, global_test_set: bool = False) -> Tuple[DataLoader, DataLoader]:
...
def train(net, trainloader, epochs, device, run_config: Dict):
    ...
    net.to(device)
    optimizer = torch.optim.AdamW(net.parameters(), lr=run_config["learning-rate"], weight_decay=run_config["weight-decay"])
    ...
    # Initialize metrics
    train_dice = DiceScore(num_classes=run_config["num-classes"], include_background=False, input_format="index")
    train_iou = JaccardIndex(task="multiclass", num_classes=run_config["num-classes"], ignore_index=0)
    ...
    for epoch in range(epochs):
        ...
        print(f"Loss: {epoch_loss/len(trainloader):.4f}, Dice: {avg_dice:.4f}, IoU: {avg_iou:.4f}")
...
def test(net, testloader, device, run_config: Dict, viz_fn=None, server_round=None):
    ...
    val_dice = DiceScore(num_classes=run_config["num-classes"], include_background=False, input_format="index")
    val_iou = JaccardIndex(task="multiclass", num_classes=run_config["num-classes"], ignore_index=0)
    val_dice.to(device)
    val_iou.to(device)

...
class BRATSDataset2D(Dataset):
    ...
    def build_datasets(
        ...
        train_dir = os.path.join(
            base_preprocess_dir,
            f"preprocessed_{modality_to_use}_{slice_direction}_train",
        )
        test_dir = os.path.join(
            base_preprocess_dir,
            f"preprocessed_{modality_to_use}_{slice_direction}_test",
        )
    ...
    def _process_single_volume(
        data_item, data_dir, output_dir, modality_channel, slice_direction, volume_idx
    ):
        ...
        cropping_indices = BRATSDataset2D.get_cropping_indices(volume_data, margin=2)
                    ...
                    vol_metadata.append(
                        {
                            ...
                            "labels_present": [
                                int(l) for l in np.unique(label_slice) if l > 0
                            ],
                        }
                    )
...
if __name__ == "__main__":
    ...
    for idx in range(16):
        ...
        col = (
            idx % 4
        ) * 2  # Multiply by 2 because we're showing image and label side by side
        ...
        if len(unique_labels) > 1:  # If there are any non-zero labels
            print(
                f"Volume {volume_idx}, Slice {slice_idx} contains labels: {unique_labels}"
            )

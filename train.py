...
class Config:
    ...
    dataset_json_path = "/home/magnus/Datasets/Images/MedicalDecathlon/Task01_BrainTumour/dataset.json"
...
def train():
    ...
    lr_scheduler = optim.lr_scheduler.SequentialLR(
        ...
        schedulers=[
            optim.lr_scheduler.LinearLR(
                ...
                total_iters=int(0.1 * Config.epochs * len(train_loader))  # 10% of total steps for warmup
            ),
            optim.lr_scheduler.PolynomialLR(
                optimizer,
                total_iters=int(0.9 * Config.epochs * len(train_loader)),  # Remaining 90% of steps
                power=2.0
            )
        ],
        milestones=[int(0.1 * Config.epochs * len(train_loader))]  # Switch from warmup to decay at 10% of training
    )
    
    # Metrics
    train_jaccard = JaccardIndex(num_classes=Config.num_classes, task="multiclass", ignore_index=0)
    val_jaccard = JaccardIndex(num_classes=Config.num_classes, task="multiclass", ignore_index=0)
    ...
    print(f"Best model achieved at epoch {best_epoch} with Jaccard Index: {best_jaccard:.4f}")
    return model
